...
get_time_varying_estimates <- function(
  ...
  ED_bs <- map(
    1:n_bootstraps,
    \(i) ED_linelist_subset %>% sample_n(n(), replace = TRUE)
  ) %>%
    bind_rows(.id = "bootstrap") %>%
    mutate(bootstrap = as.numeric(bootstrap))
  ...
  time_varying_estimates <- left_join(
    cases_bs %>%
    ...
    complete(bootstrap, age_group,
             ...
             fill = list(n_case = 0, n_hosp = 0, n_ED = 0, n_ICU = 0)) %>%
    
    arrange(date_onset) %>% 
    group_by(bootstrap, age_group) %>%
    
    mutate(
      pr_ED = zoo::rollsum(n_ED, 7, fill = "extend") / zoo::rollsum(n_case, 7, fill = "extend"),
    ...
    select(bootstrap, age_group, date_onset, pr_hosp, pr_ICU, pr_ED, pr_age_given_case)
